...
public class ClassCommandHandler implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("escolher") || args[0].equalsIgnoreCase("choose")) {
            ...
            if (!plugin.getAvailableClasses().containsKey(className)) {
                player.sendMessage(ChatColor.RED + "Classe inválida! Escolha entre: minerador, cacador, pescador, guerreiro, ferreiro, artesao");
                return true;
            }
            ...
            if (data != null && data.getClassName() != null) {
                ...
                player.sendMessage(ChatColor.YELLOW + "Você já é um " + data.getClassName() + 
                                   " de nível " + data.getLevel() + ".");
                player.sendMessage(ChatColor.RED + "Mudar para " + className + 
                                   " irá resetar parte do seu progresso!");
                player.sendMessage(ChatColor.YELLOW + "Digite '/classe confirmar' para confirmar a mudança.");
    ...
    private void showQuestInfo(Player player) {
        ...
        player.sendMessage(ChatColor.YELLOW + "Missões concluídas: " + data.getCompletedQuests().size());
    }
